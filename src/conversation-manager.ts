...
export class ConversationManager {
	...
	private loadUserData() {
		try {
			...
			const data: UserData = JSON.parse(
				fs.readFileSync(this.dataFilePath, "utf8"),
			);
			...
			if (data.seenPosts) {
				Object.entries(data.seenPosts).forEach(([subreddit, postIds]) => {
					...
					this.seenPostsBySubreddit.set(
						normalizedSubreddit,
						FifoSet.deserialize(postIds, 1000),
					);
				});
			}

			console.log(
				`Loaded ${this.userSubscriptions.size} user subscriptions and ${this.seenPostsBySubreddit.size} subreddit seen post lists`,
			);
		} catch (error) {
	...
	unsubscribeFromAllSubreddits(address: string): boolean {
		...
		this.saveUserData();
		console.log(
			`Address ${address} unsubscribed from all ${subredditCount} subreddits`,
		);
		return true;
	}
	 ...
	 */
	async sendRedditPostToSubscribers(
		post: RedditPost,
		subreddit: string,
	): Promise<boolean> {
		const normalizedSubreddit = subreddit.toLowerCase();
	...
	async sendToSpecificAddresses(message: string, addresses: string[]) {
		...
		console.log(
			`Send complete: ${sentCount} messages sent, ${skippedCount} skipped`,
		);
	}
	 ...
	 */
	private async getAddressFromConversation(
