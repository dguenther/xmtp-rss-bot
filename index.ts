...
async function main() {
	...
	console.log(
		`✓ Will check for new posts every ${postIntervalMinutes} minutes`,
	);
	...
	async function checkAndSendRedditPosts() {
		try {
			const subscribedSubreddits =
				conversationManager.getAllSubscribedSubreddits();

			if (subscribedSubreddits.length === 0) {
				console.log(
					"No active subreddit subscriptions, skipping periodic check",
				);
				return;
			}

			console.log(
				`Checking for posts from ${subscribedSubreddits.length} subscribed subreddits...`,
			);
			...
			for (const subreddit of subscribedSubreddits) {
				try {
					...
					console.log(
						`Found ${posts.length} posts from r/${subreddit}, sending to subscribers...`,
					);

					for (const post of posts) {
						const sent = await conversationManager.sendRedditPostToSubscribers(
							post,
							subreddit,
						);
						if (sent) {
							...
							console.log(
								`Skipped already sent post from r/${subreddit}: ${post.title}`,
							);
						}
					}
				} catch (error) {
	...
	for await (const message of stream) {
		...
		if (lowerCaseText === "stop") {
			const hadSubscriptions =
				conversationManager.unsubscribeFromAllSubreddits(addressFromInboxId);
			if (hadSubscriptions) {
				await conversation.send(
					"You have been unsubscribed from all subreddits and will no longer receive Reddit posts.",
				);
			} else {
		...
		if (lowerCaseText.startsWith("reddit")) {
			...
			if (parts.length === 1) {
				...
				const userSubs =
					conversationManager.getUserSubscriptions(addressFromInboxId);
				if (userSubs.length === 0) {
					await conversation.send(
						`You're not subscribed to any subreddits yet. Use "reddit <subreddit>" to subscribe to a subreddit. For example: "reddit games"`,
					);
				} else {
					await conversation.send(
						`You're subscribed to: ${userSubs.map((sub) => `r/${sub}`).join(", ")}\n\nTo get posts from a specific subreddit, use "reddit <subreddit>"`,
					);
				}
				...
				if (!isSubscribed) {
					...
					conversationManager.subscribeToSubreddit(
						addressFromInboxId,
						subreddit,
					);
					await conversation.send(
						`✅ Subscribed to r/${subreddit}! You'll now receive new posts from this subreddit.\n\nHere are some recent posts:`,
					);
				}

				// Send recent posts from this subreddit
				console.log(
					`Sending recent Reddit posts from r/${subreddit} to ${addressFromInboxId}...`,
				);
				const posts = await fetchRedditTopPosts(subreddit, postLimit);
			...
			if (parts.length === 2) {
				const subreddit = parts[1];
				const wasUnsubscribed = conversationManager.unsubscribeFromSubreddit(
					addressFromInboxId,
					subreddit,
				);
				...
				if (wasUnsubscribed) {
					await conversation.send(
						`✅ Unsubscribed from r/${subreddit}. You'll no longer receive posts from this subreddit.`,
					);
				} else {
				...
				await conversation.send(
					`Use "unsubscribe <subreddit>" to unsubscribe from a subreddit. For example: "unsubscribe games"`,
				);
			}
		} else {
